...
var EntryExit = 
{
    var entrySphere : BoundingSphere;
...
var ViewportTransform = 
{
    ...
    GetViewportRectangle : function( clientRectPos/*vector*/, clientRectDimm /*vector*/, backgroundDimensions /*vector*/ )
	{
        if (backgroundDimensions != null) {
            ...
            if (referencePos != null) 
			{
                ...
                var left = camPos.x;
                var top = camPos.y;
                var right = left + clientRectDimm.x;
                var bottom = top + clientRectDimm.y;
                ...
                else if (right >= backgroundDimensions.x) 
				{
                    left = backgroundDimensions.x - clientRectDimm.x;
                    right = backgroundDimensions.x
                }

                if (top < 0) 
				{
                    top = 0;
                    bottom = clientRectDimm.y;
                }
                else if (bottom >= backgroundDimensions.y) 
				{
                    top = backgroundDimensions.y - clientRectDimm.y;
                    bottom = backgroundDimensions.y;
                }

                return { drawingPos: {x: left, y: top}, width: (right - left), height: (bottom - top)};
            }
        }
    ...
    GetImageClipping : function( image /*img*/ )
	{
        return this.GetViewportRectangle(
    ...
    TransformPos : function( vecPos /* vector with x and y*/ )
    {
    ...
    InverseTransformPos : function(vecPos /*vec with x y */ )
    {
        var clipPos = this.GetLevelClipping().getPosition();
    ...
    DrawImageUsingViewport : function(
        context, img,
        offsetWorld,
