...
class Button implements IDrawable {
    ...
    constructor(msgId: number, msg: string) {
        this.msgId = msgId;
        this.msg = msg;
    }
    ...
    Draw(cxt: CanvasRenderingContext2D, location?: Vector) {
        ...
        //TODO draw choice icon?

        //draw option text
        cxt.fillText(this.msg, location.getX(), location.getY());
    }
}

class Chat implements IInteraction {

    private msg: IChatMsg;
    private optionButtons: Button[];
    private choiceFn: any;
    ...
    SetResultCallback(choiceFn: (chosenMsgId: number) => void): void {
        this.choiceFn = choiceFn;
    }
    ...
    Draw(cxt: CanvasRenderingContext2D, location?: Vector) {
        ...
        cxt.fillText(this.msg.msg, location.getX(), location.getY());

        //draw any buttons
        for (var i = 0; i < this.optionButtons.length; ++i) {
            location.setX(location.getX() + 10);  //TODO 10 shouldn't be hard coded.  This moves down to make more room
            this.optionButtons[i].Draw(cxt, location);
        }
    }
